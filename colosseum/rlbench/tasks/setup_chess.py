...
class SetupChess(Task):
    ...
    def init_task(self) -> None:
        ...
        self.pieces: List[Object] = [
            Shape(f"{color}_{piece}_dynamic")
            for color in COLORS
            for piece in PIECES
        ]

        self.positions = [
            piece.get_position(self.board) for piece in self.pieces
        ]
        self.rotations = [
            piece.get_orientation(self.board) for piece in self.pieces
        ]

        self.success_detectors = [
            ProximitySensor(f"detector_{file}{rank}")
        ...
        for piece, detector in zip(self.pieces, self.success_detectors):
            ...
            detector.set_position((x, y, z), self.board)  # type: ignore
    ...
    def init_episode(self, index: int) -> List[str]:
        for piece, position, rotation in zip(
            self.pieces, self.positions, self.rotations
        ):
        ...
        self.placed = -1
        self.places = random.sample(
            [(dx, dy) for dx in range(8) for dy in range(1, 3)], self.nsetup
        )
        ...
        self.indices = np.random.choice(
            np.arange(len(self.pieces)), self.nsetup, replace=False
        )
        ...
        if self.nsetup == 1:
            cmds = [
                "place the remaining chess piece in its initial"
                + " position on the board"
            ]
        else:
            cmds = [
                f"place the {self.nsetup} remaining chess pieces in"
    ...
    def _move_above_next_target(self, _):
        ...
        w4.set_position(
            [xf - 1e-3, yf, zf + 0.1], self.board, reset_dynamics=False
        )
